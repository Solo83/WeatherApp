...
public class LocationRemoveServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            user = userService.getUserByLocationId(locationId.toString(),userFromRequest.get().getId().toString());
        } catch (Exception e) {
            req.setAttribute("error", e.getMessage());
            thymeleafTemplateRenderer.renderTemplate(req, resp, "home");
            return;
        ...
        if (user.isPresent() && user.get().equals(userFromRequest.get())) {
            locationService.removeLocation(locationId, userFromRequest.get());
        }
